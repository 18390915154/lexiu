...
$(document).ready(function () {
    ...
    $.ajax({
        url: "https://www.lerlex.com/lexiu/manage/company/seachcompany",
        data: ({
        }),
        type: "post",
        dataType: "json",
        success: function (data) {
            ...
            $('#generalForm tbody').on( 'click', '.detail', function () {
                var companyid=$(this).parent().siblings().eq(0).text();
                //alert(companyid);
                window.location.replace("companydetail.html?companyid="+companyid+"");

            } );
            //编辑-----------------------------------------------------------
            $('#generalForm tbody').on( 'click', '.edit', function () {
                var companyid=$(this).parent().siblings().eq(0).text();
                window.location.replace("companyedit.html?companyid="+companyid+"");

            } );
            //查看配件--------------------------------------------------------
            // $('#generalForm tbody').on( 'click', '.fitting', function () {
            //     var companyid=$(this).parent().siblings().eq(0).text();
            //     window.location.replace("shopfitting.html?companyid="+companyid+"");
            // } );
            //删除--------------------------------------------------------
            $('#generalForm tbody').on( 'click', '.delete', function () {
                var companyid=$(this).parent().siblings().eq(0).text();
                $.ajax({
                    url: "https://www.lerlex.com/lexiu/manage/company/delcompany",
                    data: ({
                        companyid:companyid
                    }),
                    type: "post",
    ...
    $('.add').on( 'click',function () {
        window.location.replace("companyadd.html?companyid");
    } );
});
